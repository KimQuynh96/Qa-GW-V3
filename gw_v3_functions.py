...
class commons():
    ...
    def ClickElementWithXpath(xpath):
        WebDriverWait(driver, 50).until(EC.presence_of_element_located((By.XPATH,xpath))).click()
    ...
    def IsDisplayedByClass(name):
        try:
            WebDriverWait(driver, 50).until(EC.presence_of_element_located((By.CLASS_NAME,name)))
            return True
        except NoSuchElementException:
            return False
    
    
    def IsDisplayedByIdLogin():
        for i in range(2,7) :
            try :
                time.sleep(1)
    ...
    def SwitchToFrameEditor(frame_xpath):
